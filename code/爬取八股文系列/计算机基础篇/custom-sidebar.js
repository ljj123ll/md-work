...
(function() {
  ...
  function generateSidebar() {
    ...
    if (!sidebarContainer) {
      ...
      // 直接添加到body最前面，确保它显示在最前面
      document.body.insertBefore(sidebarContainer, document.body.firstChild);
      
      // 创建侧边栏标题
      const sidebarTitle = document.createElement('div');
      sidebarTitle.className = 'sidebar-title';
      sidebarTitle.textContent = '计算机基础篇 - 目录导航';
      ...
      // 尝试使用不同的选择器确保能找到标题
      const headings = document.querySelectorAll('.theme-default-content h2, .theme-default-content h3, .theme-default-content h4, h2, h3, h4');
      ...
      if (headings.length === 0) {
        // 如果没有找到标题，创建一些示例项
        const sampleItems = ['Linux', 'Network', '操作系统', 'SQL', 'MySQL', 'Redis'];
        ...
        headings.forEach(heading => {
          ...
          if (level > currentLevel) {
            ...
            // 确保parentStack不为空且有lastChild
            if (parentStack.length > 0 && parentStack[parentStack.length - 1].lastChild) {
              parentStack[parentStack.length - 1].lastChild.appendChild(subMenu);
              parentStack.push(subMenu);
              subMenu.appendChild(navItem);
            } else {
              // 如果没有合适的父元素，就直接添加到当前父元素
