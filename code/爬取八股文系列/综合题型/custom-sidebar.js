...
(function() {
  ...
  function generateSidebar() {
    ...
    if (!sidebarContainer) {
      ...
      // 先隐藏VuePress默认的侧边栏
      const vuePressSidebar = document.querySelector('.sidebar');
      if (vuePressSidebar) {
        vuePressSidebar.style.display = 'none';
      }
      ...
      // 查找页面中的标题元素（增加多种选择器确保能找到标题）
      const headings = document.querySelectorAll('#app .content__default h1, #app .content__default h2, #app .content__default h3, #app .content__default h4, .content__default h1, .content__default h2, .content__default h3, .content__default h4, h1, h2, h3, h4');
