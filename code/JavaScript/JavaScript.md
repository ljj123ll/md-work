...
| ------------------- | ------ | ------------------------------------------------------------ | ---------------------------------------- |
| 微任务（Microtask） | 高     | 1. Promise.then/catch/finally2. async/await（本质是 Promise 语法糖）3. queueMicrotask()4. MutationObserver（DOM 变化监听） | 同步任务执行完后，**立即清空所有微任务** |
| 宏任务（Macrotask） | 低     | 1. 全局 `script` 代码（整体是一个宏任务）2. setTimeout/setInterval3. DOM 事件（如 click、input）4. fetch/AJAX 网络请求5. setImmediate（Node 环境）6. requestAnimationFrame（浏览器环境，视觉相关） | 所有微任务清空后，**执行一个宏任务**     |
